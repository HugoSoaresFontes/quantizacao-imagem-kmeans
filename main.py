...
# Label de cada pixel da imagem
print "Predicao das cores com k-means"
t0 = time()
labels = kmeans.predict(imagem_array)
print "Processo feito em %0.3fs." % (time() - t0)


def reconstruir_imagem(codebook, labels, w, h):
    d = codebook.shape[1]
    imagem = np.zeros((w, h, d))
    label_idx = 0
    for i in range(w):
        for j in range(h):
            imagem[i][j] = codebook[labels[label_idx]]
            label_idx += 1
    return imagem

def vetor_diferenca(original, quantizada):
    ''' Retorna um vetor com a diferença de cada pixel da imagem original
        em relacação a imagem quantizada
    '''
    w, h, d = imagem.shape
    ...
    for i in range(w-10):
        for j in range(h):
            for k in range(d):
                vetor_diferenca[index] += abs(original[i,j,k] - quantizada[i,j,k])
                index += 1
    return vetor_diferenca

imagem_quantizada = reconstruir_imagem(kmeans.cluster_centers_, labels, w, h)
...
plt.figure("Quantizada")
plt.clf()
plt.title('Quantizada com ' + str(n_clusters) + ' Clusters')
plt.imshow(imagem_quantizada)

# Plot das Clusters/Cores
fig = plt.figure("Cores dos clusters")
ax = fig.add_subplot(111, projection='3d')
ax.set_title("Cores Clusters")
...
for cor in kmeans.cluster_centers_:
    ...
    c = '#%02x%02x%02x' % c
    ax.scatter(cor[0]*255, cor[1]*255, cor[2]*255, c=c, s=100)
